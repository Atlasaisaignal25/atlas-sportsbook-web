    ...
    >
      ...
      {showCompletedResult && completedResult?.icon ? (
        <span
          ...
          className={`inline-flex h-6 w-6 translate-y-1.5 -translate-x-2 items-center justify-center self-center justify-self-center rounded-[8px] border text-[12px] font-black ${completedResult.badgeClassName}`}
        >
...
function MyAtlasRankingRow({ row, isLast }: { row: MyAtlasBoardRow; isLast: boolean }) {
  ...
  return (
    ...
    >
      <div className="grid place-items-center gap-1">
        <div className="relative h-10 w-12">
          <div className="absolute left-0 top-1">
            <TeamBadge teamName={row.awayTeam} sport={row.sport} />
          </div>
          <div className="absolute right-0 top-1">
            <TeamBadge teamName={row.homeTeam} sport={row.sport} />
          </div>
        </div>
        <span className="max-w-[72px] truncate text-[8px] font-black uppercase tracking-[0.03em] text-white/58">
          {row.sport === "SOCCER" ? "SOCCER" : row.sport}
        </span>
      </div>

      <div className="min-w-0">
        <p className="truncate text-[13px] font-black text-white">
          {getDisplayName(row.awayTeam)} vs {getDisplayName(row.homeTeam)}
        </p>
        <p className="truncate text-[13px] font-semibold text-cyan-300">
          {formatDisplayedPick(row.pick, row.sport)}
          {oddsLabel !== "N/A" ? <span className="ml-1 text-white/58">{oddsLabel}</span> : null}
        </p>
        <p className="mt-0.5 truncate text-[9px] font-black uppercase tracking-[0.08em] text-white/38">
          {row.source}{row.rank ? ` #${row.rank}` : ""}{row.startTime ? ` · ${formatTime(row.startTime)}` : ""}
        </p>
      </div>
      ...
      <span className="text-2xl font-light text-white/54">›</span>
    </div>
  );
}

function MyAtlasRankingSection({
  title,
  subtitle,
  rows,
  empty,
  ...
  const accentClass =
    accent === "gold"
      ? "text-amber-300 bg-amber-300/12"
      : accent === "purple"
      ? "text-purple-300 bg-purple-300/12"
      : "text-cyan-300 bg-cyan-400/12";
  const titleClass = accent === "gold" ? "text-amber-300" : accent === "purple" ? "text-purple-300" : "text-cyan-300";
  ...
  return (
    <section className="relative isolate overflow-hidden rounded-[17px] border border-white/14 bg-[linear-gradient(180deg,rgba(6,18,31,0.86),rgba(3,8,20,0.92))]">
      <div className="relative z-20 flex items-center justify-between gap-2 border-b border-white/10 bg-[rgba(5,12,24,0.94)] px-3 py-2.5 backdrop-blur-sm">
        <div className="min-w-0">
          <div className="flex min-w-0 items-center gap-2">
            <h2 className={`shrink-0 text-[14px] font-black uppercase tracking-[0.18em] ${titleClass}`}>{title}</h2>
            <span className={`truncate rounded-full px-2.5 py-1 text-[9.5px] font-black ${accentClass}`}>
              {rows.length} Signals
            </span>
          </div>
          <p className="mt-0.5 truncate text-[10px] font-semibold text-white/48">{subtitle}</p>
        </div>
      </div>
      ...
      {lockedMessage ? (
        ...
        <div className="px-4 py-7 text-center">
          <p className="text-[13px] font-black text-white">{empty}</p>
          <p className="mt-1 text-[12px] font-semibold text-white/48">Atlas will display this board when eligible signals are available.</p>
        </div>
      )}
    </section>
  );
...
function mapMyAtlasBoardRow(pick: Top5Entry, sport: CheckoutSport, source: MyAtlasBoardRow["source"], index: number): MyAtlasBoardRow {
  return {
    id: `${source}-${sport}-${String(pick.gameId ?? pick.rank ?? index)}`,
    sport,
    awayTeam: pick.awayTeam ?? "",
    homeTeam: pick.homeTeam ?? "",
    pick: pick.pick,
...
type HomeMembershipDetailCode = "exclusive" | "premium" | "unlimited" | "top_signal";

type HomeMembershipDetail = {
  code: HomeMembershipDetailCode;
  title: string;
  subtitle: string;
  price: string;
  period: string;
...
const homeMembershipDetails: Record<HomeMembershipDetailCode, HomeMembershipDetail> = {
  exclusive: {
    code: "exclusive",
    title: "Exclusive",
    subtitle: "One Selected Sport",
    ...
    howItWorks: ["Atlas analyzes the Signals available for your selected sport.", "You receive up to three Official Ranked Signals when they meet the Atlas standard.", "If only one or two qualify, Atlas shows only those instead of filling the service with lower-quality Signals."],
    ...
    comparison: ["Focused one-sport ranked access.", "Monthly membership. Top Signal remains separate."],
    faqs: [
  ...
  premium: {
    ...
    howItWorks: ["Signal quantity can adapt by sport, event volume, available quality and Atlas distribution.", "Atlas does not promise a fixed count when the real slate varies.", "The ranked collection reflects the official Signals available for that sport."],
    sports: ["One selected sport"],
    tools: ["Atlas AI Rankings", "Market Impact", "Atlas Bankroll", "Signal History", "Closing Status", "Live Updates"],
    notIncluded: ["Top Signal is not included.", "Top Signal is a separate daily purchase."],
    comparison: ["One-sport ranked intelligence with Atlas AI Rankings.", "Monthly membership. Daily Top Signal remains separate."],
    faqs: [
  ...
  unlimited: {
    ...
    howItWorks: ["Atlas provides the official ranked collections available across all active sports.", "Signal quantity can vary by sport and slate.", "Atlas prioritizes quality and never fills collections artificially."],
    sports: ["Every available Atlas sport", "Automatic access to new sports when available"],
    tools: ["Atlas AI Rankings", "Market Impact", "Atlas Bankroll", "Signal History", "Closing Status", "Live Updates"],
    notIncluded: ["Top Signal is not included.", "Top Signal is a separate daily purchase by sport."],
    comparison: ["Ranked Signals across the full Atlas sports ecosystem.", "Monthly membership. Daily Top Signal remains separate."],
    faqs: [
  ...
  top_signal: {
    ...
    howItWorks: ["Top Signal is a daily purchase.", "The user selects one sport and receives the official Top Signal available for that day.", "Purchased Top Signals can be tracked in Atlas Bankroll and remain available in purchased Signal history."],
    ...
    notIncluded: ["No Market Impact.", "No Official Ranked Signals from monthly memberships.", "Not a monthly subscription."],
    comparison: ["Daily access to the highest-rated official Atlas Signal.", "Separate from Exclusive, Premium and Atlas Unlimited."],
    faqs: [
...
function HomeMembershipDetailView({
  detail,
  onBack,
  onCheckout,
  checkoutDisabled,
}: {
  detail: HomeMembershipDetail;
  onBack: () => void;
  onCheckout: () => void;
  checkoutDisabled?: boolean;
}) {
  const styles = homeMembershipStyles[detail.accent];
    ...
    <section className={`relative rounded-[18px] border p-3 pb-20 ${styles.shell}`}>
      ...
      <header className="grid grid-cols-[46px_1fr] gap-3">
        <span className={`grid h-11 w-11 place-items-center rounded-[15px] border shadow-[0_0_16px_currentColor] ${styles.icon}`}>
          <HomeMembershipIcon type={detail.icon} className="h-6 w-6" />
        </span>
        <div className="min-w-0">
      ...
      <div className="mt-4 rounded-[15px] border border-white/10 bg-black/22 p-3">
        <div className="flex items-end justify-between gap-3">
          ...
          <button
            ...
            className={`h-11 shrink-0 rounded-[12px] border px-4 text-[10px] font-black uppercase tracking-[0.06em] disabled:opacity-60 focus:outline-none focus:ring-2 focus:ring-white/35 ${styles.button}`}
          >
      ...
      <div className="mt-4 space-y-3">
        <HomeDetailBlock title="What Includes" items={detail.includes} tone={styles.check} />
        <HomeDetailBlock title="How It Works" items={detail.howItWorks} />
        <HomeDetailBlock title="Sports Included" items={detail.sports} />
        <HomeDetailBlock title="Available Tools" items={detail.tools} />
        <HomeDetailBlock title="What Is Not Included" items={detail.notIncluded} negative />
        <HomeDetailBlock title="Quick Comparison" items={detail.comparison} />
        ...
        <button
          ...
          onClick={onCheckout}
          disabled={checkoutDisabled}
          className={`h-11 w-full rounded-[13px] border px-3 text-[11px] font-black uppercase tracking-[0.08em] disabled:opacity-60 focus:outline-none focus:ring-2 focus:ring-white/35 ${styles.button}`}
        >
          {detail.cta} — {detail.price} {detail.period}
        </button>
      </div>
      ...
      <div className="fixed inset-x-3 bottom-[calc(env(safe-area-inset-bottom)+10px)] z-30 mx-auto max-w-[420px] rounded-[16px] border border-white/12 bg-[#050816]/96 p-2 shadow-[0_0_24px_rgba(34,211,238,0.18)] backdrop-blur">
        <div className="grid grid-cols-[1fr_auto] items-center gap-2">
          ...
          <button
            ...
            onClick={onCheckout}
            disabled={checkoutDisabled}
            className={`h-10 rounded-[12px] border px-4 text-[10px] font-black uppercase tracking-[0.06em] disabled:opacity-60 focus:outline-none focus:ring-2 focus:ring-white/35 ${styles.button}`}
          >
            {detail.cta}
          </button>
        </div>
...
function HomeDetailBlock({ title, items, tone = "text-cyan-300", negative = false }: { title: string; items: string[]; tone?: string; negative?: boolean }) {
  return (
    <section className="rounded-[14px] border border-white/10 bg-white/[0.035] p-3">
  ...
  <section className="space-y-1.5">
    <div className="text-center">
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2">
        ...
        <h2 className="text-[21px] font-black uppercase tracking-[-0.035em] text-white">
          Choose Your Plan
        </h2>
        <span className="h-px bg-cyan-300/22" />
      </div>
    ...
    {selectedMembershipDetail ? (
      <HomeMembershipDetailView
        detail={homeMembershipDetails[selectedMembershipDetail]}
        checkoutDisabled={checkoutPlan !== null}
        onBack={() => setSelectedMembershipDetail(null)}
        onCheckout={() => {
          if (selectedMembershipDetail === "top_signal") {
            void handleSubscribe(topSignalProductForSport(selectedPackSport), selectedPackSport);
            return;
          }

          void handleSubscribe(selectedMembershipDetail, selectedMembershipDetail === "unlimited" ? undefined : selectedPackSport);
        }}
      />
    ) : (
      <>
    <article
      ...
      onClick={() => setSelectedMembershipDetail("top_signal")}
      onKeyDown={(event) => {
        if (event.key === "Enter" || event.key === " ") {
          event.preventDefault();
          setSelectedMembershipDetail("top_signal");
        }
      }}
      className="relative cursor-pointer overflow-hidden rounded-[13px] border border-amber-300/75 bg-[radial-gradient(circle_at_left,rgba(251,191,36,0.24),rgba(217,119,6,0.08)_42%,rgba(5,8,22,0.94))] p-2 shadow-[0_0_20px_rgba(245,158,11,0.18)] focus:outline-none focus:ring-2 focus:ring-amber-300/45"
    >
      <span className="absolute left-0 top-0 rounded-br-[10px] border-b border-r border-amber-300/55 bg-amber-400/18 px-2 py-0.5 text-[6.5px] font-black uppercase tracking-[0.08em] text-amber-100">Daily Access</span>
      <div className="mt-4 grid grid-cols-[30px_1fr_auto] items-center gap-1.5">
        <span className="grid h-8 w-8 place-items-center rounded-full border border-amber-300/45 bg-amber-300/12 text-amber-200 shadow-[0_0_16px_rgba(245,158,11,0.24)]">
          <HomeMembershipIcon type="star" className="h-5 w-5" />
        </span>
        <div className="min-w-0">
        ...
        <div className="text-right">
          ...
          <p className="text-[7px] font-semibold text-white/62">/ day</p>
          <p className="mt-0.5 text-[5.5px] font-semibold leading-[7px] text-white/62">One-Time<br />Daily Access</p>
        </div>
      </div>
      ...
      <button
        ...
        onClick={(event) => {
          event.stopPropagation();
          handleSubscribe(topSignalProductForSport(selectedPackSport), selectedPackSport);
        }}
        disabled={checkoutPlan !== null}
        className="mt-1.5 h-7 w-full rounded-[9px] border border-amber-300/55 bg-gradient-to-r from-amber-500 via-yellow-300 to-amber-200 text-[7.5px] font-black uppercase tracking-[0.08em] text-black shadow-[0_0_16px_rgba(245,158,11,0.22)] disabled:opacity-60"
      >
        Unlock Top Signal
      </button>
      ...
      {homeMembershipPlans.map((plan) => {
        ...
        return (
          <article
            ...
            onClick={() => setSelectedMembershipDetail(plan.plan)}
            onKeyDown={(event) => {
              if (event.key === "Enter" || event.key === " ") {
                event.preventDefault();
                setSelectedMembershipDetail(plan.plan);
              }
            }}
            className={`relative flex h-[284px] min-w-0 cursor-pointer flex-col rounded-[11px] border px-2 pb-2 pt-2.5 text-left focus:outline-none focus:ring-2 focus:ring-white/30 ${styles.shell}`}
          >
            {plan.badge ? (
            ...
            <div className="mt-auto pt-1">
              ...
              <button
                ...
                onClick={(event) => {
                  event.stopPropagation();
                  handleSubscribe(plan.plan, plan.plan === "unlimited" ? undefined : selectedPackSport);
                }}
                disabled={checkoutPlan !== null}
