...
function JoinPlanIcon({ type, className = "" }: { type: "target" | "crown" | "diamond" | "star"; className?: string }) {
  if (type === "target") {
    return (
      <svg viewBox="0 0 64 64" aria-hidden="true" className={className} fill="none">
        <circle cx="29" cy="35" r="18" stroke="currentColor" strokeWidth="5" />
        <circle cx="29" cy="35" r="8" stroke="currentColor" strokeWidth="5" />
        <path d="M29 35 49 15" stroke="currentColor" strokeWidth="5" strokeLinecap="round" />
        <path d="M45 9h10v10M42 22h10v10" stroke="currentColor" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    );
  }
  ...
  if (type === "crown") {
    return (
      <svg viewBox="0 0 64 64" aria-hidden="true" className={className} fill="currentColor">
        ...
        <path d="M12 26 25 37 32 18l7 19 13-11-4 22H16l-4-22Z" />
        <path d="M17 51h30l-3 6H20l-3-6Z" />
      </svg>
    );
  }
  ...
  if (appSection === "more") {
    const joinPlanCards = [
      ...
      },
      {
        ...
        title: "Premium",
        subtitle: "Choose Your Sport",
        featureTitle: "Ranked Top 3 Signals",
    ...
    const joinToneStyles = {
      cyan: {
        card: "border-cyan-300/50 bg-cyan-400/[0.045] shadow-[0_0_22px_rgba(34,211,238,0.12)]",
        text: "text-cyan-300",
        panel: "border-cyan-300/20 bg-cyan-300/10",
        button: "border-cyan-300/75 text-cyan-200",
        check: "text-cyan-300",
      },
      gold: {
        card: "border-amber-300/60 bg-amber-400/[0.055] shadow-[0_0_26px_rgba(251,191,36,0.16)]",
        text: "text-amber-300",
        panel: "border-amber-300/24 bg-amber-300/12",
        button: "border-amber-300 bg-amber-300 text-black",
        check: "text-amber-300",
      },
      purple: {
        card: "border-purple-300/55 bg-purple-500/[0.055] shadow-[0_0_22px_rgba(192,132,252,0.13)]",
        text: "text-purple-300",
        panel: "border-purple-300/22 bg-purple-300/12",
      ...
      <main className="min-h-screen bg-[#020715] text-white">
        <div className="mx-auto min-h-screen w-full max-w-md bg-[#020715]">
          <div className="relative min-h-screen overflow-hidden">
            ...
            />
            <div className="absolute inset-x-0 top-[300px] h-40 bg-gradient-to-b from-transparent via-[#020715]/85 to-[#020715]" />
            <div className="absolute inset-0 bg-gradient-to-b from-transparent via-[#020715]/20 to-[#020715]" />

            <div className="relative z-10 pb-6">
              <header className="flex h-[74px] items-center justify-center border-b border-white/8 px-4 pt-2">
                <button
                  type="button"
                  onClick={() => navigateAppState({ section: "signals", view: "live", sport: "TOP" })}
                  aria-label="Back to signals"
                  className="absolute left-4 top-7 grid h-9 w-9 place-items-center rounded-full text-[28px] leading-none text-white/82"
                >
                  ‹
                </button>
                <p className="text-[13px] font-black uppercase tracking-[0.42em] text-white">
                  Join Atlas <span className="text-cyan-300">Signals</span>
                </p>
              </header>

              <section className="px-4 pt-6">
                <h1 className="max-w-[230px] text-[34px] font-black leading-[0.95] tracking-tight text-white">
                  Unlock Premium Signals.
                </h1>
                <p className="mt-5 max-w-[190px] text-[18px] font-semibold leading-[1.2] text-white/68">
                  More clarity. Less risk. Better results.
                </p>
              </section>

              <section className="mt-7 px-4">
                <div className="rounded-[20px] border border-white/12 bg-[#07111d]/88 p-4 shadow-[0_0_28px_rgba(34,211,238,0.08)] backdrop-blur-md">
              ...
              <section className="mt-6 px-4 text-center">
                <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-4">
                  ...
                  <h2 className="text-[19px] font-black uppercase tracking-[0.26em] text-cyan-300">Choose Your Plan</h2>
                  <span className="h-px bg-cyan-300/22" />
                </div>
                <p className="mt-2 text-[12px] font-semibold text-white/50">Pick the plan that matches your game.</p>

                <div className="mt-4 grid grid-cols-3 gap-2">
                  {joinPlanCards.map((plan) => {
                    const styles = joinToneStyles[plan.tone as keyof typeof joinToneStyles];

                    return (
                      <article key={plan.plan} className={`relative flex min-h-[320px] flex-col rounded-[16px] border px-2 pb-2 pt-5 text-left ${styles.card}`}>
                        {plan.badge ? (
                          <span className="absolute left-1/2 top-[-8px] -translate-x-1/2 whitespace-nowrap rounded-full bg-amber-300 px-2 py-0.5 text-[7px] font-black uppercase tracking-[0.06em] text-black">
                            {plan.badge}
                          </span>
                        ) : null}
                        <div className="flex justify-center">
                          <span className={`grid h-12 w-12 place-items-center rounded-full border border-current bg-white/[0.03] ${styles.text}`}>
                            <JoinPlanIcon type={plan.icon} className="h-7 w-7" />
                          </span>
                        </div>
                        <h3 className={`mt-4 text-center text-[15px] font-black uppercase tracking-[0.12em] ${styles.text}`}>{plan.title}</h3>
                        <p className="mt-1 text-center text-[10px] font-bold text-white/78">{plan.subtitle}</p>
                        <div className={`mt-3 rounded-[10px] border px-1.5 py-2 text-center ${styles.panel}`}>
                          <p className="text-[10px] font-black leading-tight text-white">{plan.featureTitle}</p>
                          <p className={`mt-1 text-[10px] font-black leading-tight ${styles.text}`}>{plan.featureSubtitle}</p>
                        </div>
                        <div className="mt-3 space-y-2">
                          {plan.features.map((feature) => (
                            <p key={feature} className="grid grid-cols-[11px_1fr] gap-1 text-[8.5px] font-semibold leading-tight text-white/70">
                        ...
                        <button
                          ...
                          disabled={checkoutPlan !== null}
                          className={`mt-auto h-10 rounded-[10px] border text-[8.5px] font-black uppercase tracking-[0.08em] disabled:opacity-60 ${styles.button}`}
                        >
                          {plan.cta}
                        </button>
                      </article>
              ...
              <section className="mt-6 px-4">
                <div className="rounded-[18px] border border-cyan-300/12 bg-cyan-300/[0.035] p-3">
                  <div className="mb-3 grid grid-cols-[1fr_auto_auto] items-center gap-3">
                    <span className="h-px bg-white/10" />
                    <h2 className="text-[16px] font-black uppercase tracking-[0.20em] text-cyan-300">Premium Add-ons</h2>
                    <span className="text-[8px] font-black uppercase tracking-[0.10em] text-white/42">Not Included in Subscriptions</span>
                  </div>
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (!authSession.authenticated) {
                          setJoinAuthMode("signup");
                          setJoinAuthOpen(true);
                          setJoinAuthMessage({ tone: "info", title: "Create your account first.", body: "Sign up here, then unlock Top Signal inside Atlas Signals." });
                          return;
                        }
                        void handleSubscribe(topSignalProductForSport(selectedPackSport), selectedPackSport);
                      }}
                      disabled={checkoutPlan !== null}
                      className="rounded-[14px] border border-purple-300/35 bg-purple-400/[0.055] p-3 text-left disabled:opacity-60"
                    >
                      <div className="flex items-center gap-2">
                        <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full border border-purple-300/45 text-purple-200">
                          <JoinPlanIcon type="star" className="h-6 w-6" />
                        </span>
                        <div>
                          <p className="text-[12px] font-black uppercase tracking-[0.12em] text-purple-300">Top Signal</p>
                          <p className="mt-1 text-[17px] font-black text-white">$24.99 <span className="text-[9px] text-white/50">/day</span></p>
                        </div>
                      </div>
                      <p className="mt-3 text-[9px] font-semibold leading-4 text-white/68">The #1 strongest signal of the day for a specific sport.</p>
                      <span className="mt-3 flex h-8 items-center justify-center rounded-[9px] border border-purple-300/45 text-[9px] font-black uppercase text-purple-200">Unlock</span>
                    </button>
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (!authSession.authenticated) {
                          setJoinAuthMode("signup");
                          setJoinAuthOpen(true);
                          setJoinAuthMessage({ tone: "info", title: "Create your account first.", body: "Sign up here, then unlock Top Play inside Atlas Signals." });
                          return;
                        }
                        void handleSubscribe("top_play");
                      }}
                      disabled={checkoutPlan !== null}
                      className="rounded-[14px] border border-amber-300/35 bg-amber-400/[0.055] p-3 text-left disabled:opacity-60"
                    >
                      <div className="flex items-center gap-2">
                        <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full border border-amber-300/45 text-amber-300">
                          <JoinPlanIcon type="crown" className="h-6 w-6" />
                        </span>
                        <div>
                          <p className="text-[12px] font-black uppercase tracking-[0.12em] text-amber-300">Top Play</p>
                          <p className="mt-1 text-[17px] font-black text-white">$149.99</p>
                        </div>
                      </div>
                      <p className="mt-3 text-[9px] font-semibold leading-4 text-white/68">The highest-conviction play selected by Atlas Signals.</p>
                      <span className="mt-3 flex h-8 items-center justify-center rounded-[9px] border border-amber-300/45 text-[9px] font-black uppercase text-amber-200">Unlock</span>
                    </button>
                  </div>
                </div>
              </section>
              ...
              <section className="mt-6 px-4">
                <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-4">
                  ...
                  <h2 className="text-[17px] font-black uppercase tracking-[0.24em] text-cyan-300">Compare Plans</h2>
                  <span className="h-px bg-cyan-300/20" />
                </div>
              ...
              <section className="mt-5 px-4 pb-6">
                <div className="grid grid-cols-3 gap-2 rounded-[18px] border border-cyan-300/14 bg-cyan-300/[0.035] p-3">
                  {[
                    ["24/7", "24/7 Updates", "Real-time signals and market scans", "text-cyan-300"],
                    ["🏆", "Winning Edge", "AI-powered analysis for sharp results", "text-amber-300"],
                    ["♢", "Cancel Anytime", "No long-term contracts. You’re in control.", "text-emerald-300"],
                  ].map(([icon, title, body, color]) => (
                    <div key={title} className="min-w-0 border-r border-white/10 pr-2 last:border-r-0">
                      <p className={`text-[16px] font-black ${color}`}>{icon}</p>
                      <p className="mt-1 text-[10px] font-black text-white/72">{title}</p>
                      <p className="mt-0.5 text-[8px] font-semibold leading-3 text-white/42">{body}</p>
                    </div>
                  ))}
                </div>
              </section>
