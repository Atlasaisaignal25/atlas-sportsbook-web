...
function formatDisplayedPick(rawPick: string, sport: string) {
  ...
  const totalMatch = pick.match(/^(over|under)\s*[\(\s]?([0-9]+(?:\.[0-9]+)?)\)?$/i);
  if (totalMatch) {
    const side = totalMatch[1].toLowerCase() === "over" ? "Over" : "Under";
    return `${side} ${totalMatch[2]}`;
  }

  // SPREAD sin paréntesis (Team -1.5)
...
function findPick(game: OddsGame, sport: string) {
  if (sport === "MLB") {
    ...
    const top5Match =
      mlbTop5.top5.find((g) => isSameMatch(game, g)) || null;
  ...
  if (sport === "NBA") {
    ...
    const top5Match =
      nbaTop5.top5.find((g) => isSameMatch(game, g)) || null;
  ...
  if (sport === "NHL") {
    ...
    const top5Match =
      nhlTop5.top5.find((g) => isSameMatch(game, g)) || null;
...
export default function Home() {
  ...
  useEffect(() => {
    const sport = sportsMap[selectedSport];
...
const topSignals = [
  { ...mlbTop5.top5?.[0], sport: "MLB" },
  { ...nbaTop5.top5?.[0], sport: "NBA" },
  { ...nhlTop5.top5?.[0], sport: "NHL" },
  { ...soccerTop5.top5?.[0], sport: "SOCCER" },
].filter(Boolean);
...
{pick.startTime && (
  ...
  {formatDisplayedPick(pickData.pick, selectedSport)}
                    ...
                    {pickData.isTop5 && !pickData.isTopSignal && userPlan === "premium" && (
                      <span className="inline-block rounded-full bg-yellow-500/20 px-2 py-1 text-[10px] font-semibold text-yellow-300">
